...
test_ext_node_read_cb(ReadNodeContext *ctx, ExtensibleNode *node)
{
	...
	token = pg_strtok(ctx, &length);	/* skip :relid */
	token = pg_strtok(ctx, &length);	/* get value */
	tnode->relid = atooid(token);

	token = pg_strtok(ctx, &length);	/* skip :repeat_count */
	token = pg_strtok(ctx, &length);	/* get value */
...
test_create_custom_scan_state(CustomScan *cscan)
{
	 ...
	 * a plain scan uses, so this never introduces an extra conflict.
	 */
					  ...
					  Index rti, RangeTblEntry *rte)
{
	 ...
	 * real provider would estimate the cost honestly. The row count, on the
	 * other hand, is reported truthfully: we hand back each scanned row
	 * repeat_count times.
	 */
 ...
 *
 * The result is checked rather than blindly cast: these functions are
 * reachable by any user, and a string describing some other kind of node must
 * produce an ERROR instead of a misinterpreted struct.
 */
static TestExtNode *
