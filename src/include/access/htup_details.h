...
HeapTupleCopyBaseFromPage(HeapTuple tup, void *page)
{
	...
	{
		tup->t_xmin = HeapTupleHeaderXminFrozen(tup_hdr) ?
					  FrozenTransactionId :
					  ShortTransactionIdToNormal(
							  HeapPageGetSpecial(page)->pd_xid_base, tup_hdr->t_choice.t_heap.t_xmin);
		tup->t_xmax = ShortTransactionIdToNormal(
				(tup_hdr->t_infomask & HEAP_XMAX_IS_MULTI) ? HeapPageGetSpecial(page)->pd_multi_base : HeapPageGetSpecial(page)->pd_xid_base,
