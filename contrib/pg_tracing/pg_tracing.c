...
/* Current trace state */
struct pgTracingTrace current_trace = {};
...
/* Start timestamp of the last executor finish */
static TimestampTz executor_finish_start_ts;
...
typedef struct pgTracingPerLevelBuffer
{
	...
	int			parse_span_index;	/* Index of the latest parse span in the
									 * current_trace_spans */
...
/*
 * Add current trace context to the parallel workers shared buffer.
 */
static void
...
/*
 * Get parse span for the provided nested level
 */
static Span *
get_parse_span_from_nested_level(int nested_level, bool force_allocation)
{
	Span	   *parse_span;

	if (force_allocation && per_level_buffers[nested_level].parse_span_index != -1)
	{
		parse_span = get_span_from_index(per_level_buffers[nested_level].parse_span_index);
		Assert(parse_span->ended == true);
	}
	if (force_allocation || per_level_buffers[nested_level].parse_span_index == -1)
		per_level_buffers[nested_level].parse_span_index = get_index_from_trace_spans();
	parse_span = get_span_from_index(per_level_buffers[nested_level].parse_span_index);
	return parse_span;
}

/*
 * End the trace of the span at the provided index
 */
static void
 ...
 */
static void
end_top_span(int nested_level, const int64 *end_span_ns)
{
	Assert(nested_level <= max_nested_level);
...
/*
 * Add span to the shared memory. Mutex should already have been acquired.
 */
static void
...
check_traceparameter_in_parameter(ParamListInfo params)
{
	...
	current_trace = extract_traceparent(pstring, true);
}
...
check_query_sampled(ParseState *pstate, const ParamListInfo params)
{
	...
	if (params != NULL)
		...
		current_trace = extract_traceparent(pstate->p_sourcetext, false);
...
/*
 * Adjust spans' offsets to the provided file offset
 */
static void
 ...
 */
static int64
get_top_span_start(void)
{
	/* For the top span of parallel worker, we get the current ns */
	if (exec_nested_level == 0 && IsParallelWorker())
	...
	/*
	 * We're in a nested query, use set nested start. This is only set if
	 * we're inside a ProcessUtility. Other nested queries (like function
	 * call) will have their start/duration adjusted when queryDesc is
	 * processed.
	 */
...
/*
 * End of the query tracing, dump all of top spans in the shared
 * memory. This may happen either when query is finished or on a caught error.
 */
static void
...
handle_pg_error(int span_index, QueryDesc *queryDesc)
{
	...
	/* End current span and ongoing top spans */
 ...
 *
 * nested_level can be negative when creating the top top span.
 * In this case, we want to use the trace parent id.
 */
static uint64
...
initialize_trace_level(void)
{
	...
	if (max_nested_level == -1)
	{
		...
		/*
		 * We need to be able to pass informations that depend on the nested
		 * level.
		 *
		 ...
		 *
		 * parse spans: We don't know the parent of nested parse span until we
		 * process the plan node. We keep them in this buffer to be able to
		 * change their parent.
		 */
		oldcxt = MemoryContextSwitchTo(pg_tracing_mem_ctx);
		/* initial allocation */
		...
		for (int i = 0; i < allocated_nested_level; i++)
		{
			...
			per_level_buffers[i].parse_span_index = -1;
		}
		...
		if (!IsParallelWorker())
		{
			 ...
			 * so let's estimate it from here.
			 */
 ...
 */
static int64
initialize_trace_level_and_top_span(CmdType commandType, Query *query, JumbleState *jstate, const char *query_text)
{
	bool		new_nested_level = initialize_trace_level();
	int64		top_span_start_ns = get_top_span_start();
...
pg_tracing_post_parse_analyze(ParseState *pstate, Query *query, JumbleState *jstate)
{
	...
	bool		leave_parse_span_open = false;
	...
	end_parse_ns = start_post_parse_ns - 1000;
	 ...
	 */
	top_span_start_ns = initialize_trace_level_and_top_span(query->commandType, query, jstate, pstate->p_sourcetext);

	parent_id = get_parent_id(exec_nested_level);
	Assert(parent_id > 0);

	if (top_span_start_ns == 0)
	{
		/*
		 * We are inside a nested query that wasn't started from
		 * ProcessUtility. We will rely on reparent span at the executor end
		 * to set the parse start and duration will be used to store the parse
		 * span end.
		 */
		top_span_start_ns = end_parse_ns;
		leave_parse_span_open = true;
		/* TODO: Handle this? */
	}

	if (!leave_parse_span_open)
	{
		/*
		 * We need to keep track of the parse span. Within nested queries, and
		 * outside of processUtility, we only know of the start of the parse
		 * span when we process planstate nodes at the end of the Executor. In
		 * this case, top_span_start_ns is set to 0
		 */
		parse_span = get_parse_span_from_nested_level(exec_nested_level, true);
		...
		/*
		 * In case of queries nested in a result, we don't have the parse
		 * start. We will need to update this span when processing the
		 * queryDesc.
		 */
	...
	if (parse_span != NULL && parse_span->start + parse_span->duration_ns / NS_PER_US >= post_parse_span->start)

		/*
		 * We want to avoid parse spans ending at the exact same microsend as
		 * the post parse span. Artificially increase post parse time if it's
		 * the case
		 */
		post_parse_span->start++;
						...
						ParamListInfo params)
{
	...
	/*
	 * We may have skipped parsing if statement was prepared, start a new top
	 * span if we don't have one
	 */
	if (current_trace.sampled == 1 && pg_tracing_enabled(plan_nested_level + exec_nested_level))
		initialize_trace_level_and_top_span(query->commandType, query, NULL, query_string);
...
pg_tracing_ExecutorStart(QueryDesc *queryDesc, int eflags)
{
	...
	if (current_trace.sampled > 0 && pg_tracing_enabled(exec_nested_level))
	{
		 ...
		 * nor planner hook.
		 *
		 * Parallel workers will also start tracing from there.
		 */
		initialize_trace_level_and_top_span(queryDesc->operation, NULL, NULL, queryDesc->sourceText);
 ...
 * If the plan needs to create parallel workers, push the trace context in parallel shared buffer.
 */
static void
					   ...
					   uint64 count, bool execute_once)
{
	...
	if (current_trace.sampled > 0 && pg_tracing_enabled(exec_nested_level))
	{
		...
		span = get_span_from_index(span_index);
		begin_span(span, SPAN_EXECUTOR_RUN, &current_trace, get_parent_id(exec_nested_level), per_level_buffers[exec_nested_level].query_id, NULL, exec_nested_level);

		/*
		 * Executor run is used as the parent's span for nodes, store it's id
		 * in the per level buffer.
		 */
		per_level_buffers[exec_nested_level].executor_id = span->span_id;
		if (queryDesc->plannedstmt->parallelModeNeeded)
			add_parallel_context(span->span_id, per_level_buffers[exec_nested_level].query_id);
	}
	 ...
	 * be disabled while we have an ongoing trace. Check for
	 * current_trace_spans
	 */
...
pg_tracing_ExecutorFinish(QueryDesc *queryDesc)
{
	...
	if (current_trace.sampled > 0 && pg_tracing_enabled(exec_nested_level))
	{
		...
		executor_finish_start_ts = span->start;
	}
...
pg_tracing_ExecutorEnd(QueryDesc *queryDesc)
{
	...
	if (current_trace.sampled > 0 && pg_tracing_enabled(exec_nested_level))
	{
		...
		span_index = get_index_from_trace_spans();
		span = get_span_from_index(span_index);
		begin_span(span, SPAN_EXECUTOR_END, &current_trace, get_parent_id(exec_nested_level), per_level_buffers[exec_nested_level].query_id,
				   NULL, exec_nested_level);

		/*
		 * Executor finish can have a sub microsencond duration leading to
		 * both finish and end to start at the same timestamp. Artificially
		 * inscreate the start of the finish span if it's the case.
		 */
		if (span->start == executor_finish_start_ts)
			span->start++;
	...
	if (span_index > -1)
	{
		...
		int64		end_child_span_ns = end_span_ns - 1000;

		if (exec_nested_level == 0)
			end_top_span(exec_nested_level, &end_span_ns);
		/* End the matching top span */
		end_span_index(span_index, &end_child_span_ns);
		end_tracing(&end_span_ns);
	}
}
 ...
 * the top span and parse span parent. We will only know and create their
 * parent when we generate span node from planstate.
 *
 * When those nodes are created, we attach the parse spans and top spans to them and
 * adjust their start/duration.
 */
static void
reparent_child_query(Span const *parent_span, int64 start_span_ns)
{
	Span	   *child_parse_span;
	Span	   *child_top_span;

	if (exec_nested_level + 1 > max_nested_level)
		/* No child to reparent */
		return;

	/* Use the same start/duration as the parent */
	child_top_span = get_top_span_for_nested_level(exec_nested_level + 1);
	Assert(child_top_span->ended == false);
	child_top_span->parent_id = parent_span->span_id;
	child_top_span->start = parent_span->start;
	child_top_span->duration_ns = parent_span->duration_ns;
	child_top_span->ended = true;

	if (per_level_buffers[exec_nested_level + 1].parse_span_index == -1)
		/* No parse span started */
		return;

	child_parse_span = get_parse_span_from_nested_level(exec_nested_level + 1, false);
	if (child_parse_span->trace_id > 0)
	{
		int64		end_parse_span_ns;

		/*
		 * We've stored the end ns of the parse span in the duration_ns field.
		 * We can match the parse start with it's parent and compute the
		 * duration with the elements we have.
		 */
		end_parse_span_ns = child_parse_span->duration_ns;
		child_parse_span->start = parent_span->start;
		child_parse_span->duration_ns = start_span_ns;
		end_span(child_parse_span, &end_parse_span_ns);
		Assert(child_parse_span->duration_ns > 0);
	}
}
 ...
 * during parse step.
 *
 * Explain statements are skipped
 */
static void
						  ...
						  DestReceiver *dest, QueryCompletion *qc)
{
	 ...
	 * We artificially start nested a bit later to avoid overlapping spans due
	 * to rounding
	 */
	...
	/*
	 * Set ProcessUtility span as the new top span just in case we have nested
	 * query. We need to keep the old top span index to be able to close it.
	 */
	old_top_span_index = per_level_buffers[exec_nested_level].top_span_index;
	...
	{
		exec_nested_level--;
		handle_pg_error(process_utility_span_index, NULL);
		PG_RE_THROW();
	}
	PG_END_TRY();
	end_span_ns = get_current_ns();

	/* ProcessUtility may have created a top span, close it here. */
	...
	/* buffer may have been repalloced */
	process_utility_span = get_span_from_index(process_utility_span_index);
	if (qc != NULL)
		process_utility_span->node_counters.rows = qc->nprocessed;
	/* ProcessUtility is the current top span, no need to call end_top_span */
	end_child_span_ns = end_span_ns - 1000;
	end_span(process_utility_span, &end_child_span_ns);

	/* Also end the old top span */
	end_span_index(old_top_span_index, &end_span_ns);
	end_tracing(&end_span_ns);
				 ...
				 SpanType span_type, char *subplan_name)
{
	...
	node_duration_ns = INSTR_TIME_GET_NANOSEC(planstate->instrument->firsttime);
	Assert(planstate->instrument->total > 0);
	node_duration_ns += planstate->instrument->total * NS_PER_S;
	end_span(span, &node_duration_ns);
	...
	if (nodeTag(plan) == T_Result)
		reparent_child_query(span, start_span_ns);
	return span->span_id;
}
...
generate_span_from_planstate(PlanState *planstate, planstateTraceContext * planstateTraceContext, uint64 parent_id)
{
	...
	if (planstate->lefttree)
		generate_span_from_planstate(planstate->lefttree, planstateTraceContext, span_id);
	if (planstate->righttree)
		generate_span_from_planstate(planstate->righttree, planstateTraceContext, span_id);
...
/*
 * Add plan counters to Datum output
 */
static int
...
/*
 * Add node counters to Datum output
 */
static int
...
add_result_span(ReturnSetInfo *rsinfo, Span * span,
				const char *qbuffer, Size qbuffer_size, Relation rel)
{
	...
	values[i++] = CStringGetTextDatum(get_span_name(span, qbuffer, qbuffer_size));
	values[i++] = CStringGetTextDatum(get_operation_name(span, qbuffer, qbuffer_size));
	values[i++] = Int64GetDatum(span->start);

	values[i++] = Int64GetDatum(span->duration_ns);
	...
	values[i++] = BoolGetDatum(span->top_span);
...
pg_tracing_spans(PG_FUNCTION_ARGS)
{
	 ...
	 * query file.
	 *
	 * Better abort the sampling, clean ongoing traces and remove any possible
	 * parallel context pushed
	 *
	 * Since this will be called within an ExecutorRun, we will need to check
	 * for current_trace_spans later.
	 */
	cleanup_tracing();
...

	{
		...
		for (int i = 0; i < shared_spans->end; i++)
		{
			...
			add_result_span(rsinfo, span, qbuffer, qbuffer_size, NULL);
		}
...
pg_tracing_reset(PG_FUNCTION_ARGS)
{
	 ...
	 */
	{
		...
		s->stats.traces = 0;
		s->stats.spans = 0;
		s->stats.dropped_spans = 0;
		s->stats.failed_truncates = 0;
		s->stats.last_consume = 0;
