...
static void
ExplainTargetRel(const PlanState *planstate, const Plan *plan, Index rti, StringInfo *str)
{
	...
	rtable_names = select_rtable_names_for_explain(rtable, NULL);

	refname = (char *) list_nth(rtable_names, rti - 1);
	if (refname == NULL)
...
static void
ExplainScanTarget(const PlanState *planstate, const Scan *plan, StringInfo *str)
{
	ExplainTargetRel(planstate, (Plan *) plan, plan->scanrelid, str);
}

static void
ExplainModifyTarget(const PlanState *planstate, const ModifyTable *plan, StringInfo *str)
{
	ExplainTargetRel(planstate, (Plan *) plan, plan->nominalRelation, str);
}
...
static void
plan_to_rel_name(const PlanState *planstate, const Plan *plan, StringInfo *str)
{
	switch (nodeTag(plan))
	{
		case T_SeqScan:
		...
		case T_CteScan:
		case T_WorkTableScan:
			ExplainScanTarget(planstate, (Scan *) plan, str);
			break;
		case T_ForeignScan:
		case T_CustomScan:
		...
		case T_IndexScan:
			{
				...
				ExplainIndexScanDetails(indexscan->indexid,
										...
										str);
				ExplainScanTarget(planstate, (Scan *) indexscan, str);
			}
			break;
		case T_IndexOnlyScan:
			{
				...
				ExplainIndexScanDetails(indexonlyscan->indexid,
										...
										str);
				ExplainScanTarget(planstate, (Scan *) indexonlyscan, str);
			}
			break;
		case T_BitmapIndexScan:
			...
			break;
		case T_ModifyTable:
			ExplainModifyTarget(planstate, (ModifyTable *) plan, str);
			break;
		case T_NestLoop:
		case T_MergeJoin:
		case T_HashJoin:
...
char const *
plan_to_operation(const PlanState *planstate, const char *spanName)
{
	...
	appendStringInfoString(operation_name, spanName);

	plan_to_rel_name(planstate, plan, &operation_name);
	return operation_name->data;
}

const char *
plan_to_span_name(const Plan *plan)
{
