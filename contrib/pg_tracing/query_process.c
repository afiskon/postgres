...
extract_traceparent(const char *sqlcomment_str, bool is_parameter)
{
	const char *expected_start = "/*";
	const char *traceparent;
	const char *end_sqlcomment;
	char	   *endptr;
	pgTracingTraceparentParameter pgTracingTraceparentParameter;

	pgTracingTraceparentParameter.sampled = 0;
	pgTracingTraceparentParameter.trace_id = 0;
	pgTracingTraceparentParameter.sampled = 0;
	...
	if (!is_parameter)
	{
		...
		for (size_t i = 0; i < strlen(expected_start); i++)
		{
			...
			{
				return pgTracingTraceparentParameter;
			}
		}
	 ...
	 */
	errno = 0;
	pgTracingTraceparentParameter.trace_id = strtol(&traceparent[3], &endptr, 16);
	if (endptr != traceparent + 35 || errno)
	{
		return pgTracingTraceparentParameter;
	}
	pgTracingTraceparentParameter.parent_id = strtol(&traceparent[36], &endptr, 16);
	if (endptr != traceparent + 52 || errno)
	{
		return pgTracingTraceparentParameter;
	}
	pgTracingTraceparentParameter.sampled = strtol(&traceparent[53], &endptr, 16);
...
const char *
normalise_query(const char *query, int *query_len_p)
{
	char	   *norm_query;
	int			query_len = *query_len_p;
...
bool
text_store(pgTracingSharedState * pg_tracing, const char *query, int query_len,
		   Size *query_offset)
{
	Size		off;
	int			fd;

	{
		...
		SpinLockAcquire(&s->file_mutex);
		off = s->extent;
		s->extent += query_len + 1;
		s->n_writers++;
		SpinLockRelease(&s->file_mutex);
	}
